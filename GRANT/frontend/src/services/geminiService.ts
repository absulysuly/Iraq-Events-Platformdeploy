...
// The API key is now injected by the Vite build process from the .env file.
...
if (!API_KEY) {
  ...
  console.warn("AI features will be disabled. To enable them, ensure the VITE_GEMINI_API_KEY environment variable is configured correctly in your .env file in the project root.");
}
...
export const generateEventDetailsFromPrompt = async (
    ...
    if (!API_KEY) {
        throw new Error("AI features are disabled. Please configure the VITE_GEMINI_API_KEY environment variable.");
    }
