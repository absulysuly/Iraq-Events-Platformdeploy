...
// Changed from Vite-specific `import.meta.env` to `process.env`.
...
if (!API_KEY) {
  ...
  // FIX: Updated warning message to align with guidelines (do not ask user to edit code).
  console.warn("AI features will be disabled. To enable them, ensure the API_KEY environment variable is configured correctly.");
}
...
export const generateEventDetailsFromPrompt = async (
    ...
    if (!API_KEY) {
        ...
        throw new Error("AI features are disabled. Please configure the API_KEY environment variable.");
    }
