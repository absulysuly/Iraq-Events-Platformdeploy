...
import { WhatsAppIcon, FacebookIcon, GmailIcon, StarIcon, BookmarkIcon } from './icons';
...
const EventDetailModal: React.FC<EventDetailModalProps> = ({ event, onClose, lang, onAddReview, currentUser, onEdit, onViewProfile, isBookmarked, onToggleBookmark }) => {
  ...
  const t = {
    ...
    bookmark: { en: 'Bookmark', ar: 'حفظ', ku: 'نیشانکردن' },
    bookmarked: { en: 'Bookmarked', ar: 'محفوظ', ku: 'نیشانکرا' }
  };
  ...
  const gmailLink = `mailto:?subject=${encodeURIComponent(event.title[lang] || event.title.en)}&body=${encodeURIComponent(`Check out this event: ${event.title[lang]}\n\n${event.description[lang]}\n\nDate: ${new Date(event.date).toLocaleString()}`)}`;
    ...
    <div className="fixed inset-0 bg-black bg-opacity-70 flex justify-center items-center z-50 p-4" role="dialog" aria-modal="true" aria-labelledby="event-detail-modal-title">
      <div className="bg-gray-800 text-gray-200 rounded-lg shadow-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto">
        <div className="relative">
            <LazyImage src={event.imageUrl} alt={event.title[lang] || event.title.en} className="w-full h-64 rounded-t-lg" />
          ...
          <div className="flex justify-between items-start gap-4">
            ...
            <div className="flex-shrink-0 flex items-center gap-2">
                {currentUser && !isOwner && (
                    ...
                    className={`flex items-center gap-2 px-4 py-2 rounded-md text-sm font-semibold transition-colors ${isBookmarked ? 'bg-teal-500/20 text-teal-300 hover:bg-teal-500/30' : 'bg-gray-600 text-gray-200 hover:bg-gray-500'}`}
                    >
                    <BookmarkIcon className="w-4 h-4" isFilled={isBookmarked} />
                    {isBookmarked ? t.bookmarked[lang] : t.bookmark[lang]}
                    </button>
                )}
                {isOwner && (
                <button 
                  ...
                  <div className="space-y-3 max-h-48 overflow-y-auto pr-2">
                    {event.reviews.map(review => (
                        <div key={review.id} className="flex gap-3">
                            <LazyImage src={review.user.avatarUrl} alt={review.user.name} className="w-10 h-10 rounded-full" />
                            <div>
